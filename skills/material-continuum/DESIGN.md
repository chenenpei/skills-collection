...
响应式网页的文字阅读纸面保持单一文字轨道：标题、正文和小标题共享同一条左侧基线；移动端不能只给正文增加额外内缩。窄屏检查至少覆盖 390px 和 680px。
...
- 叙事插画：场景与文案共同表达一个过程或概念。图片下方由一张白色叙事纸面承载 kicker、标题、正文与强调短语，并跨过图片边界形成单一阅读表面；3:4/1:1 采用上下跨界，16:9 采用左右跨界。强调短语显式使用 emphasisText；图片中的蓝色不自动成为文字色。其他版式不复用这张跨界纸面。
- 结构对照：相同维度、相同内部层级和对齐线；两张图主体尺度相称，颜色差异具有表达作用。16:9 的图文栏中，正文留在对应标题下方的文字轨道，不随网格自动掉到图片轨道。
- 结构展开：连续解释相关信息；用场景或图示承担关系。平级插图保持一致外底，不单独给中间项加亮色背景制造优先级。1:1 的独立图文条目采用单列三行，插图与文字共享顶部基线；3:4 和 16:9 保持各自已校准的密度。
- 卡片列表：同类短信息在一张纸面内形成连续行，分隔线只画在行间，末项不留尾线；16:9 两栏排列时，每一栏的底部项目都不留尾线。
- 文字阅读：保留完整论述和舒适的段落间距。短文字仍需足够纸面高度和底部余量。16:9 保留较窄的阅读行宽，但标题、正文和小标题从列表页继承同一条左侧网格线。
...
制作结构页前查看适用参照：[结构对照](assets/reference-comparison.jpg)、[结构展开](assets/reference-development.jpg)、[文字阅读](assets/reference-reading.jpg)。它们只说明层级、分组、密度、留白和纸面关系；参照用于比较构图，正文来自当前 brief，交付页面由 HTML 排版。
