...
- **Batch funnel run** — CN, US, or CN+US funnel + Deep batch for a user-specified quarter
- **Partial step** — landmine pricing, audit-summary triage, output review, or spec explain for one ticker
...
**Always prefer the CLI** for funnel, explain, validate, filter-breakdown, bank-indicators, and landmine steps. Do **not** re-implement funnel logic by calling East Money/Yahoo APIs directly unless the CLI command fails after a genuine install/`npm install` attempt — then report the error and fall back to `spec/` rules with explicit `N/A` fields.
