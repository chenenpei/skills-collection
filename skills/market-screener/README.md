...
**优先已兑现的生意。** 主质量筛选偏好已有持续盈利、股东回报和现金兑现记录的公司，为进一步研究竞争优势与护城河缩小范围。成长投入与护城河并不互斥，但依赖未来增长或再投入兑现才能证明价值的公司，不因增长故事获得更高优先级。定量通过只是财务证据，不能证明护城河存在；独立资产、金融破净与盈利修复策略保留另一类价值研究入口。

这一选择降低对未来经营预测和逐公司审计的依赖，也接受漏掉部分优秀成长或再投入型公司的代价。评估漏选时先区分计算／口径错误、数据不足与取向之外的机会。这些取舍属于本 skill，不约束独立的定性审计。

**研究名单价格档优先。** 已通过主研究资格的公司，按下文参考价格档决定研究先后，再比较同档回报等信号。“价格低”指相对参考盈利的价格吸引力，不是每股报价小；偏贵但经营更稳的公司不会因此越过价格正常的公司。这优先照顾潜在安全边际，也接受较便宜、近期承压的公司可能先被研究的代价。近期事实供复核，目前不改变排序；价格档本身也不是内在价值认证。

维护或实验先保留上述产品偏好。改变覆盖取向或价格档优先顺序，需要用户明确调整目标；修复事实错误和验证同档排期效果，与改变偏好分开处理。候选更多、个别指定股票入选或审计后可立即买入，都不作为筛选质量改善的充分证据。

**独立寻找折价机会。** 质量研究关注持续盈利与现金兑现，折价研究关注资产或保守盈利相对价格的吸引力。两类机会使用独立容量，避免业务风险暂难判断的公司挤占质量研究名额，也避免主质量失败直接排除有可靠数字支持的修复机会。折价资格依然要求必要数据可靠，专门业务风险留给后续研究。

金融破净入口同时要求账面折价与正盈利，以减少单凭低PB纳入亏损公司的情况；这一边界接受漏掉未破净的金融机会。非金融分别使用净流动资产覆盖和七年保守盈利，前者关注资产余量，后者容纳不破净但盈利价格有吸引力的公司。账面资产不等于可变现资产，历史盈利不等于未来可分配现金。
...
`run --help` 是参数约束的权威入口。`--policy` 使用 `cn-screening` 或 `template-screening`。`--no-pdf` 关闭 PDF 兜底；`--evaluate-all` 只用于观察后续条件，不改变资格和采集预算。默认主研究名单最多 30 家，另有独立折价备选最多 30 家，合计最多 60 家。分别用 `--limit`、`--backup-limit` 调整，0 关闭对应名单。
...
| `financial_discount` | 金融破净修复：可靠基本数字与PB／盈利价格条件通过，专门风险仍待核验 |
| `earnings_repair` | 非金融七年保守盈利修复：独立于主质量条件，价格必须通过 |
| `ncav` | 非金融账面净流动资产折价条件通过，价格是必要条件 |

`all` 的主研究名单包含质量或金融研究条件通过的公司；独立备选包含质量未通过或未知、但 NCAV、金融破净或盈利修复条件通过的公司。默认分别最多30和30家，不凑数、不借位：主名单满30仍可另列30家备选；只有20家主候选、3家备选就展示23家。同一公司只出现一次，保留全部命中策略；主候选因容量未展示时，也不会借备选名额。`financial` 使用相同独立容量与排序，但只包含其适用策略。
...
前30名是有限研究容量下的排期，不保证公司质量依次递减。同档历史回报较高的公司可能先于更便宜或近期经营更稳的公司；价格下降也可能伴随可持续盈利下降。固定规则随价格更新反映机会变化，不自动识别市场环境或切换策略。迭代优先修复事实和实现错误；排序调整应有跨时点、重复出现的研究反馈支持，不以单批候选的主观偏好反复调参。

备选先列NCAV，按净流动资产／市值降序；再将金融破净与非金融盈利修复按折后盈利收益率降序混排。同收益率时金融PB较低者先列，没有PB排序值的非金融随后，最后代码并列。三种策略共用独立备选容量，不按行业或策略分配名额；NCAV优先是证据类型取舍，不证明风险更低。收益率混排用于优先发现相对盈利更便宜的研究线索，但金融年度盈利与非金融七年盈利的正常化程度不同，不能据此认定质量相等；经营承压者也可能因低倍数排在前面。每家公司保留全部命中依据，仅占一个席位。
...
| 周期企业 | 明确资源、商品化农业、海运／航空货运、半导体／面板／光伏／电池及地产项目采用较长窗口 |
| 银行、券商 | 使用各自盈利及监管风险条件 |
...
普通主营和普通报表一致时可支持非金融方法。收费资管、支付、金融信息服务须有结构化业务证据；客户资产不能当作自有缓冲。末级行业名没有“金融”、或仅有“综合”标签，都不足以证明非金融。无法确认财务方法的新标签或实质冲突保持未决，不默认回落到制造业。

已核实非金融方法且使用普通报表的公司，默认采用五年基础窗口；仅因行业标签宽泛、未命中细类映射，不要求再证明所有业务都“非周期”。明确的周期行业或主营证据触发七年附加条件，主营中的货运承运业务与码头装卸、货代、轮渡、设备供应分别判断。默认窗口在结果中标记为 `standard_nonfinancial_window`，只表示不额外要求七年历史，不证明经营稳定或具有护城河，也不豁免五年核心数据缺失。

真实周期冲突、明确未决的范围判断仍保留，可以分别计算五年和七年：都通过才通过，都失败才失败，否则未知。未被现有映射识别的周期暴露可能漏掉七年检查，这是采用简单定量初筛的局限；应根据可复用的业务识别证据改进映射，而非逐家公司审计或因标签不熟悉一律划为未知。
...
默认要求最近三年报告归母利润各年为正、最新完整年报基本 EPS 与普通股 BPS 为正，并同时满足 PB ≤ 1、报告 EPS 打七折后的收益率 ≥ 7%（相当于报告 PE ≤ 10）。阈值见 `strategies.financialDiscount`，是研究线索参数，不是已验证的内在价值或清算安全边际。
...
`earnings_repair`适用于已核验的非金融方法及普通年度报表；金融明确不适用、方法未定或冲突保持未知。最新年报超过18个月未更新时保持未知。它不继承主质量、现金、杠杆或周期通过资格，也不要求PB破净。

取连续七个完整财年的普通股归属利润 `E = min(PNI, ANI)`，必须有七年可核验观测，至少五年为正且最新年为正。参考盈利为七年中位数、五年中位数、三年中位数和最新E的最小值。最新合并及归母权益须为正。默认将参考盈利打七折，再除以当前同权普通股市值，要求收益率至少8%（相当于折扣前参考盈利倍数不高于8.75）；参数见`strategies.earningsRepair`，缺价格不赋予资格。

允许两年亏损以容纳修复机会，取较低盈利基数以减少按景气高点估价的风险。七年窗口用于观察较长时间的经营波动，不保证覆盖完整周期；七折与8%是保守研究门槛，不能保证抵御持续衰退。

复用年度上下文、重述、已知非普通股收益分配、审计意见和报价／股本核对。已知非标审计排除；缺年、核心冲突保持未知，不以季度填年或缺值填零。`ER.history`与`ER.price`记录年度利润、参考基数、折扣、原始事实ID及界限，`signal`说明七年口径，便于与金融年度盈利区别。

默认全部策略运行时，七年年度指标由同一次结构化API请求取得；银行等金融方法仍仅使用自身所需年度。主质量报表仍取五年，只有周期需要时才补旧两年利润／现金流报表。修复基础数字通过后独立请求报价与股本，主质量失败不阻止此路径。接口未返回必要年份时在已有预算内记未知，不逐票开发解析器或扩大PDF抓取。
...
清单按交易所标明的数据日期核对时效。休市日可使用已核验的最近交易日清单；交易日的旧清单不视为当日完整覆盖。较旧或无法确认时效时，保留已取得身份并标记覆盖不完整。日历覆盖范围及公告来源见[交易日历](cli/src/cn/sources/exchange-calendar.json)，跨年判断需覆盖所涉年份。

年报须能核对到正式公告的公司和年度。身份或年度冲突的报告不采用；证据不足以完成核对时保留数据缺口，不等同于已确认报告错误。
...
这些是停止上限，不是预计耗时。`--budget-file` 可覆盖财务预算，并发用 `--enrich-concurrency` 调整；预算写入账本，运行中不重置。某策略已可靠失败后停止专属补数，其他仍可能通过的策略继续。`all` 在首个年度指标请求中一次取得七年历史；基础三表仍为五年。基础财报之后优先取得必要价格和股本，再补周期策略需要的旧年三表或有限 PDF；五年基础三表不完整时不先追旧年三表。
...
`results.jsonl` 与 `explain` 的 `result.recentFinancials` 保存期间、披露／更新日期、四项本期与同期金额、变化及原始事实 ID。`candidates` 也在每家公司旁展示归档已有的 `recentFinancials`，包含未获展示名额的完整候选；不会补采数据。未采集近期财务或不适用的公司可以没有该字段，不表示近期稳定。`yoy` 是比例（-0.2 表示下降 20%），仅在同期基数为正时提供；零或负基数显示金额变化及 `nonpositive_prior`。`hints` 描述下降、转负、归母上升而扣非下降等事实，不推断原因，也不影响资格、价格分档或排序。`complete` 只表示这四项数据可比，不表示经营健康；`missing`、`stale`、`conflicting` 分别表示缺数据、仍未取得应披露的新一期、口径或金额冲突。缺失近期信息不会推翻已通过的年度资格。
...
`replay` 使用归档保存的实现、政策和来源重现并核对结果，要求依赖与归档记录一致。`run --input` 则使用当前实现和政策重新评估兼容的证据输入；两者都不重新采集。当前解析器未支持的表格数值或解释性上下文，在来源及定位校验通过后保留为 `unverified_field_or_scope_contract`，撤去可参与计算的值，不阻断其他可靠事实。来源被改动、定位不符、可直接比较的数值矛盾及已支持口径的校验错误仍拒绝导入。提前停止的采集可能没有后续条件所需的事实，重新评估时仍须保留这些缺口。
