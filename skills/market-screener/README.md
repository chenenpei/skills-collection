...
**优先已兑现的生意。** 主质量筛选偏好已有持续盈利、股东回报和现金兑现记录的公司，为进一步研究竞争优势与护城河缩小范围。成长投入与护城河并不互斥，但依赖未来增长或再投入兑现才能证明价值的公司，不因增长故事获得更高优先级。定量通过只是财务证据，不能证明护城河存在；独立资产／金融折价策略保留另一类价值研究入口。
...
`results.jsonl` 与 `explain` 的 `result.recentFinancials` 保存期间、披露／更新日期、四项本期与同期金额、变化及原始事实 ID。`candidates` 也在每家公司旁展示归档已有的 `recentFinancials`，包含未获展示名额的完整候选；不会补采数据。旧归档或不适用的公司可以没有该字段，不表示近期稳定。`yoy` 是比例（-0.2 表示下降 20%），仅在同期基数为正时提供；零或负基数显示金额变化及 `nonpositive_prior`。`hints` 描述下降、转负、归母上升而扣非下降等事实，不推断原因，也不影响资格、价格分档或排序。`complete` 只表示这四项数据可比，不表示经营健康；`missing`、`stale`、`conflicting` 分别表示缺数据、仍未取得应披露的新一期、口径或金额冲突。缺失近期信息不会推翻已通过的年度资格。
