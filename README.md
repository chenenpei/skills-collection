...
| **stock-analysis-audit** | 对个股和基金类产品做证据优先的投资审计。个股侧重商业模式、财务质量、护城河、条件式价格分层、机会成本和对抗分析；基金侧覆盖 ETF、指数基金、主动基金等，重点看基金结构、持仓暴露、费用、同类基金比较和组合角色。支持轻量审计与深度审计，兼容 Lite／Deep。`references/` 集中研究规则与报告要求，`chatbot.md` 提供可直接复制的提示词，使用说明和示例见技能 README。结论仅供研究辅助，非投资建议。 |
...
本地安装 Material Continuum：

```bash
npx skills add . --skill material-continuum -y
```
