...
| **market-screener** | A 股与美股单公司季度定量漏斗：Kill Gate → 行业路由 → sector 模板打分，产出 `candidates.yaml` 等工件并衔接 **stock-analysis-audit** Deep。含 `spec/` 可执行规则、手动触发的 `SKILL.md` 编排，以及 `cli/` 下 TypeScript `screener` 命令（validate / run / explain / landmine / filter-breakdown）。详见 [CONTEXT-MAP.md](./CONTEXT-MAP.md)。 |
